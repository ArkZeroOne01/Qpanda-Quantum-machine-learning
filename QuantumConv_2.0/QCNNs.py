...
def qcnn_circuit(x, weights, num_qubits, num_clist):
    ...
    cir = pq.QProg()
    for i in range(num_qubits):  # 激发态
        cir.insert(pq.H(qubits[i]))
    cir.insert(pq.RZ(qubits[0], x[0]))  # 相位编码
    cir.insert(pq.RZ(qubits[1], x[3]))
    cir.insert(pq.RZ(qubits[2], x[12]))
    cir.insert(pq.RZ(qubits[3], x[15]))
    
    for i in range(num_qubits):  # 参数相位
        cir.insert(pq.RY(qubits[i], weights[i]))
        
    for i in range(num_qubits - 1):  # 建立纠缠
        cir.insert(pq.CNOT(qubits[i], qubits[i + 1]))
    cir.insert(pq.CNOT(qubits[num_qubits - 1], qubits[0]))

    cir.insert(pq.RZ(qubits[0], x[0]))  # 相位编码
    cir.insert(pq.RZ(qubits[1], x[2]))
    cir.insert(pq.RZ(qubits[2], x[8]))
    cir.insert(pq.RZ(qubits[3], x[10]))
    ...
    cir.insert(pq.RZ(qubits[0], x[0]))  # 相位编码
    cir.insert(pq.RZ(qubits[1], x[1]))
    cir.insert(pq.RZ(qubits[2], x[4]))
    cir.insert(pq.RZ(qubits[3], x[5]))
    ...
    for i in range(num_qubits):  # 参数相位
        cir.insert(pq.RY(qubits[i], weights[12+i]))
        cir.insert(pq.RZ(qubits[i], weights[16+i]))
    ...
    for j in range(0, 25,2):
        for k in range(0, 25,2):
            ...
            q_results = qcnn_circuit(
                [
                    x[j * 28 + k], x[j * 28 + k + 1], x[j * 28 + k + 2], x[j * 28 + k + 3],
                    x[(j + 1) * 28 + k], x[(j + 1) * 28 + k + 1], x[(j + 1) * 28 + k + 2], x[(j + 1) * 28 + k + 3],
                    x[(j + 2) * 28 + k], x[(j + 2) * 28 + k + 1], x[(j + 2) * 28 + k + 2], x[(j + 2) * 28 + k + 3],
                    x[(j + 3) * 28 + k], x[(j + 3) * 28 + k + 1], x[(j + 3) * 28 + k + 2], x[(j + 3) * 28 + k + 3]
                ],
                weights, num_qubits, num_clist
            )
            for w in range(4):
            	out[w*169+t] = q_results[w]
            t = t + 1
    return out
...
class Net(Module):
    def __init__(self):
        ...
        self.quantum_circuit = QuantumLayerMultiProcess(build_multiprocess_qmlp_circuit,5*4,4, 1, diff_method="finite_diff")
        self.maxpool1 = MaxPool2D([2, 2], [2, 2], padding="valid")
        self.fc1 = Linear(input_channels=144, output_channels=70)
        self.fc2 = Linear(input_channels=70, output_channels=10)
    def forward(self, x):
        x = tensor.flatten(x, 1)
        x = self.quantum_circuit(x)     # 1 1 25 25
        x = x.reshape((-1,4,13,13))
        x = self.maxpool1(x)		# 1 1 12 12
        x = tensor.flatten(x, 1)   # 1 144
        x = F.ReLu()(self.fc1(x))  # 1 70
        x = self.fc2(x)            # 1 10
        return x
...
for epoch in range(epochs):
    ...
    n_train = 0
    w = 0
    for x, y in data_generator(x_train, y_train, batch_size=batchsize, shuffle=True):
...
with open("train_loss_list_100.txt", "w", encoding="utf-8") as file:
    for item in train_loss_list:
        file.write(str(item) + "\n")
with open("val_loss_list_100.txt", "w", encoding="utf-8") as file:
    for item in val_loss_list:
        file.write(str(item) + "\n")
with open("train_acc_list_100.txt", "w", encoding="utf-8") as file:
    for item in train_acc_list:
        file.write(str(item) + "\n")
with open("val_acc_list_100.txt", "w", encoding="utf-8") as file:
