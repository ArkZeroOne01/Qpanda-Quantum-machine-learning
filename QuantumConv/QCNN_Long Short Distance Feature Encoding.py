...
def qcnn_circuit(x, weights, num_qubits, num_clist):
    ...
    cir.insert(pq.RZ(qubits[2], x[6]))
    cir.insert(pq.RZ(qubits[3], x[8]))
    ...
    cir.insert(pq.RZ(qubits[0], x[1]))  # 相位编码
    cir.insert(pq.RZ(qubits[1], x[3]))
    cir.insert(pq.RZ(qubits[2], x[5]))
    cir.insert(pq.RZ(qubits[3], x[7]))
    for i in range(num_qubits):  # 参数相位
    ...
    #     result4 = machine.prob_run_list(cir, [qubits[4]], -1)

    #     result = [(result0[-1]+result1[-1]+result2[-1]+result3[-1]+result4[-1])/5]
    #     machine.finalize()
    #     return result
    ...
    for j in range(0, 25, 2):
        for k in range(0, 25, 2):
            ...
            q_results = qcnn_circuit(
                [
                    x[j * 28 + k], x[j * 28 + k + 1], x[j * 28 + k + 2],
                    x[(j + 1) * 28 + k], x[(j + 1) * 28 + k + 1], x[(j + 1) * 28 + k + 2],
                    x[(j + 2) * 28 + k], x[(j + 2) * 28 + k + 1], x[(j + 2) * 28 + k + 2]
                ],
...
class Net(Module):
    def __init__(self):
        ...
        self.fc1 = Linear(input_channels=169, output_channels=80)
...
for epoch in range(epochs):
    ...
    if epoch==50:
        with open("train_loss_list_50.txt", "w", encoding="utf-8") as file:
            for item in train_loss_list:
                file.write(str(item) + "\n")
        with open("val_loss_list_50.txt", "w", encoding="utf-8") as file:
            for item in val_loss_list:
                file.write(str(item) + "\n")
        with open("train_acc_list_50.txt", "w", encoding="utf-8") as file:
            for item in train_acc_list:
                file.write(str(item) + "\n")
        with open("val_acc_list_50.txt", "w", encoding="utf-8") as file:
            for item in val_acc_list:
                file.write(str(item) + "\n")
with open("train_loss_list_0.txt", "w", encoding="utf-8") as file:
    for item in train_loss_list:
        file.write(str(item) + "\n")
with open("val_loss_list_0.txt", "w", encoding="utf-8") as file:
    for item in val_loss_list:
        file.write(str(item) + "\n")
with open("train_acc_list_0.txt", "w", encoding="utf-8") as file:
    for item in train_acc_list:
        file.write(str(item) + "\n")
with open("val_acc_list_0.txt", "w", encoding="utf-8") as file:
    for item in val_acc_list:
        file.write(str(item) + "\n")
